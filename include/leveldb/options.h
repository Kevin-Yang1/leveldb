...
#include <cstddef> // 引入 C 标准库，定义了像 size_t 这样的类型

#include "leveldb/export.h" // 引入用于控制符号可见性的宏

namespace leveldb {

// 前向声明，这些类将在 Options 结构体中作为指针成员类型使用
class Cache;         // 用于缓存数据块
class Comparator;    // 用于比较键
class Env;           // 抽象了操作系统环境，如文件系统操作
class FilterPolicy;  // 用于布隆过滤器等策略，以减少磁盘读取
class Logger;        // 用于记录日志信息
class Snapshot;      // 代表数据库在特定时间点的只读视图

// 数据库内容存储在一系列的数据块中，每个数据块包含一系列的键值对。
// 每个数据块在存储到文件之前可能会被压缩。
// 以下枚举描述了用于压缩数据块的压缩方法（如果有的话）。
enum CompressionType {
  // 注意：不要更改现有条目的值，因为它们是磁盘上持久格式的一部分。
  kNoCompression = 0x0,     // 不进行压缩
  kSnappyCompression = 0x1, // 使用 Snappy 压缩算法
  kZstdCompression = 0x2,   // 使用 Zstd 压缩算法
};

// 控制数据库行为的选项（传递给 DB::Open 时使用）
struct LEVELDB_EXPORT Options {
  // 创建一个具有所有字段默认值的 Options 对象。
  Options();
  ...
  // 在转换为排序的磁盘文件之前，在内存中构建的数据量（由磁盘上的未排序日志支持）。
  //
  ...
  // 数据库可以使用的打开文件数。
  // 如果您的数据库具有较大的工作集（为工作集的每 2MB 预算一个打开文件），
  // 您可能需要增加此值。
  int max_open_files = 1000; // 最大打开文件数，默认为 1000

  // 对数据块的控制（用户数据存储在一系列数据块中，数据块是从磁盘读取的单位）。

  // 如果非空，则使用指定的缓存来存储数据块。
  // 如果为空，leveldb 将自动创建并使用一个 8MB 的内部缓存。
  Cache* block_cache = nullptr;

  // 每个数据块中打包的用户数据的大致大小。请注意，此处指定的
  // 数据块大小对应于未压缩的数据。如果启用了压缩，
  // 从磁盘读取的单元的实际大小可能会更小。此参数可以动态更改。
  size_t block_size = 4 * 1024; // 数据块大小，默认为 4KB

  // 键的增量编码中重启点之间的键数。
  // 此参数可以动态更改。大多数客户端应保持此参数不变。
  int block_restart_interval = 16; // 数据块重启点间隔，默认为 16

  // LevelDB 在切换到新文件之前将向文件写入的最大字节数。
  // 大多数客户端应保持此参数不变。但是，如果您的文件系统
  // 对较大的文件更有效，您可以考虑增加该值。缺点是
  // 压缩过程会更长，因此延迟/性能抖动会更长。
  // 增加此参数的另一个原因可能是当您最初填充大型数据库时。
  size_t max_file_size = 2 * 1024 * 1024; // 最大文件大小，默认为 2MB

  // 使用指定的压缩算法压缩数据块。此参数可以动态更改。
  //
  ...
  // 许多应用程序将从此处的 NewBloomFilterPolicy() 的结果中受益。
...
struct LEVELDB_EXPORT ReadOptions {
  ...
  // 如果 "snapshot" 非空，则按提供的快照读取（该快照必须属于
  // 正在读取的数据库，并且不得已释放）。如果 "snapshot" 为空，
  // 则使用此读取操作开始时状态的隐式快照。
...
struct LEVELDB_EXPORT WriteOptions {
  ...
  // 如果为 true，则在写入被视为完成之前，写入将从操作系统
  // 缓冲区缓存中刷新（通过调用 WritableFile::Sync()）。
  // 如果此标志为 true，则写入速度会变慢。
  //
  // 如果此标志为 false，并且机器崩溃，则某些最近的
  // 写入可能会丢失。请注意，如果只是进程崩溃
  // （即机器没有重新启动），即使 sync==false，也不会丢失任何写入。
  //
  // 换句话说，sync==false 的数据库写入具有与 "write()" 系统调用
  // 相似的崩溃语义。sync==true 的数据库写入具有与 "write()"
  // 系统调用后跟 "fsync()" 相似的崩溃语义。
  // sync==false：数据可能还停留在内存，没有真正落盘，可能丢失（像只用了 write()）。
  // sync==true：数据被强制同步到磁盘，更安全（像 write() 后立刻 fsync()）。
  bool sync = false; // 是否同步写入，默认为 false
};
