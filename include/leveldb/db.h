...
#include <cstdint> // C++ 标准整数类型库
#include <cstdio>  // C 标准输入输出库

#include "leveldb/export.h"   // 用于库导出的宏定义
#include "leveldb/iterator.h" // 迭代器接口定义
#include "leveldb/options.h"  // 数据库选项定义

namespace leveldb {

// 如果更改这些版本号，请更新 CMakeLists.txt
...
// 数据库特定状态的抽象句柄。
// Snapshot 是一个不可变对象，因此可以在多个线程中安全访问，无需任何外部同步。
class LEVELDB_EXPORT Snapshot {
 protected:
  // 析构函数声明为虚函数，允许派生类正确释放资源
  virtual ~Snapshot();
};
...
// DB 是一个持久化的、从键到值的有序映射。
// DB 在没有外部同步的情况下，可以安全地被多个线程并发访问。
class LEVELDB_EXPORT DB {
 public:
  // 打开具有指定 "name" 的数据库。
  // 成功时，在 *dbptr 中存储一个指向堆分配数据库的指针，并返回 OK。
  // 出错时，在 *dbptr 中存储 nullptr，并返回一个非 OK 状态。
  ...
  // 将 "key" 对应的数据库条目设置为 "value"。成功时返回 OK，出错时返回非 OK 状态。
  // 注意：考虑设置 options.sync = true 以确保数据持久化。
  virtual Status Put(const WriteOptions& options, const Slice& key,
                     const Slice& value) = 0;

  // 删除 "key" 对应的数据库条目 (如果存在)。成功时返回 OK，出错时返回非 OK 状态。
  // 如果 "key" 在数据库中不存在，则不视为错误。
  // 注意：考虑设置 options.sync = true 以确保数据持久化。
  ...
  // 如果数据库包含 "key" 对应的条目，则将对应的值存储在 *value 中并返回 OK。
  //
  // 如果没有 "key" 对应的条目，则保持 *value 不变，并返回一个 Status::IsNotFound() 为 true 的状态。
  //
  // 可能因其他错误返回其他 Status。
  ...
  // 有效的属性名称包括：
  //
  //  "leveldb.num-files-at-level<N>" - 返回层级 <N> 的文件数量，
  //     其中 <N> 是层级号的 ASCII 表示 (例如 "0")。
  ...
  // 对键范围 [*begin,*end] 的底层存储进行 Compact 操作。
  // 特别是，已删除和已覆盖的版本将被丢弃，并且数据将被重新排列以降低访问数据所需的操作成本。
  // 此操作通常只应由了解底层实现的用戶调用。
  //
  // begin==nullptr 被视为空指针，代表数据库中所有键之前的键。
  // end==nullptr 被视为空指针，代表数据库中所有键之后的键。
  // 因此，以下调用将 Compact 整个数据库：
...
// 如果无法打开数据库，您可以尝试调用此方法来尽可能多地恢复数据库的内容。
